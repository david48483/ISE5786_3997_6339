...
public class NEWTEST {
    ...
    public static void setupScene() {
        ...
        Geometries backgroundBranch = new Geometries(
                // Floor: keeps the nice KR reflection, but dark to avoid overexposure
                ...
                // Back wall (right of image): cream color but very dim. Strong light will brighten it.
                // Placed at Z = -8 to be close and create a true right angle
               /* new Plane(new Point(0, 0, -8), new Vector(0, 0, 1))
                        .setEmission(new Color(90, 75, 65))
                        .setMaterial(new Material().setKD(0.15).setKS(0)),*/

                // Left wall (left of image): gray color, very dim.
                // Placed at X = -8, cutting the previous wall at 90 degrees
        ...
        Geometries spheres = new Geometries(
                ...
                // 1. Fully matte sphere
// placed behind the glass sphere, half hidden and half visible to the camera
                new geometries.impl.Sphere(new Point(-1.0, 1.0, 2.1), radius)
                        .setEmission(new Color(200, 50, 50))
                        .setMaterial(new Material()
                                .setKD(0.2) // absorbs less light to preserve the red color
                                .setKS(0.0)
                                .setShininess(0)),

                // 2. Mirror sphere
                // Placed on the right side (closer to positive X) floating at Y=0.5
                new geometries.impl.Sphere(new Point(2.5, 0.5, 0.0), radius)
                ...
                // 3. Frosted glass sphere
                // Placed on the left/front (positive Z, X near 0) floating at Y=1.0
                new geometries.impl.Sphere(new Point(0.0, 1.0, 2.5), radius)
...
// 4. Light beams from the car's front headlights (simulates forward light projection)
        ...
        cameraBuilder = Camera.getBuilder()
                ...
                .setRaysAmount(9)
                .setDebugPrint(0.1)
                .setResolution(800, 800); // reduced slightly so tests complete faster
    }
