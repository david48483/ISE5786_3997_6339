...
public class NEWTEST {
    ...
    public static void setupScene() {
        ...
        Geometries spheres = new Geometries(
                ...
                // 1. כדור מט לגמרי (Matte)
// מוקם כעת מאחורי כדור הזכוכית, חציו מוסתר וחציו גלוי למצלמה
                new geometries.impl.Sphere(new Point(-1.0, 1.0, 2.1), radius)
                        .setEmission(new Color(200, 50, 50))
                        .setMaterial(new Material()
                                .setKD(0.2) // סופג פחות אור כדי לשמור על הצבע האדום
                                .setKS(0.0)
                                .setShininess(0)),

                // 2. כדור מראה (Mirror)
                // מוקם בצד ימין (קרוב יותר ל-X חיובי) ומרחף בגובה Y=0.5
                ...
                // 3. כדור זכוכית חלבית (Frosted Glass)
                // מוקם בצד שמאל/קדימה (Z חיובי, X קרוב ל-0) ומרחף בגובה Y=1.0
...
// 4. אלומות אור מהפנסים הקדמיים של הרכב (מדמה זריקת אור קדימה)
        ...
        cameraBuilder = Camera.getBuilder()
                ...
                .setRaysAmount(9)
                .setDebugPrint(0.1)
                .setResolution(800, 800); // הורדתי מעט כדי שהטסטים יסתיימו מהר
    }
